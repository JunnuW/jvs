...
function fleNamer(a,b){if(1>a.length)alert("Filename is required");else{a=a.replace(/(^\/)|(\/$)/g,"");var c=/^[-_\/\sa-z0-9]+$/i;if(c.test(a)){b=b.replace(/(^\/)|(\/$)/g,"");if(c.test(b))return 0==a.indexOf(b)?a:b+"/"+a;alert("Invalid directory name: "+b)}else alert("Invalid filename: "+a)}}
function simParHeader(){var a;a={Estart:simSettn.eVstart,Estop:simSettn.eVstop,Etr:simSettn.eTr};inhomSpectr.inPlot.jdos&&(a.jdos=simSettn.jdostype);if(inhomSpectr.inPlot.Se){var b={};b.Eb_meV=simSettn.exEb;"bulk"==simSettn.jdostype?b.type="Bulk-3D":(b.type="QW-2D",b.viewDir=simSettn.viewDir,b.polarizat=simSettn.polarizat,"TM"==simSettn.polarizat&&(b.shapePar=simSettn.ex0));a.exiton=b}inhomSpectr.inPlot.fcv&&(a.Boltzmann={Epeak_eV:a.Etr,Temp_K:simSettn.kelvin});inhomSpectr.inPlot.Lurb&&(inhomSpectr.inPlot.convo?
a.InhmgBrdng={type:"AsymUrbach",Eu_meV:simSettn.eU}:a.FuncPlot={type:"AsymUrbach",Eu_meV:simSettn.eU});inhomSpectr.inPlot.Lsurb&&(inhomSpectr.inPlot.convo?a.InhmgBrdng={type:"SymmUrbach",Eu_meV:simSettn.eU}:a.FuncPlot={type:"SymmUrbach",Eu_meV:simSettn.eU});inhomSpectr.inPlot.Lgaus&&(inhomSpectr.inPlot.convo?a.InhmgBrdng={type:"Gaussian",sigma_meV:simSettn.GauSig}:a.FuncPlot={type:"Gaussian",sigma_meV:simSettn.GauSig});inhomSpectr.inPlot.Lsech&&(inhomSpectr.inPlot.convo?a.InhmgBrdng={type:"Sech^n(t/tau)",
tau_fs:simSettn.SechTau,n:simSettn.SechN}:a.FuncPlot={type:"Sech^n(t/tau)",tau_fs:simSettn.SechTau,n:simSettn.SechN});"Homogeneous simulation"==$("#lblsaveSimSpe").text()&&homSpectr.inPlot.convo&&($("#pltLorentz").prop("checked")&&(a.HomogBrdng={type:"Lorentz",tau_fs:simSettn.LorTau}),$("#pltDlorentz").prop("checked")&&(a.HomogBrdng={type:"Lorentz X Lorentz",tau1_fs:simSettn.DlTau1,tau2_fs:simSettn.DlTau2}));"Homogeneous simulation"!=$("#lblsaveSimSpe").text()||homSpectr.inPlot.convo||($("#pltLorentz").prop("checked")&&
(a.FuncPlot={type:"Lorentz",tau_fs:simSettn.LorTau}),$("#pltDlorentz").prop("checked")&&(a.FuncPlot={type:"Lorentz1 X Lorentz2",tau1_fs:simSettn.DlTau1,tau2_fs:simSettn.DlTau2}));return a}
