...
$("#mongoDialForm").dialog("close");$("#stackLocFiles").focus().click();break;case "Open emission spectrum":$("#mongoDialForm").dialog("close");$("#emisLocFiles").focus().click();break;case "Save Inhomogeneous spectrum":a=graphSettn.inhomFileN;$("#settnDial").dialog("option","title");c=experEmisDat();seiv_loucal(a,c,emisToLocFile);break;case "Save Homogeneous spectrum":a=graphSettn.homFileN;$("#settnDial").dialog("option","title");c=experEmisDat();seiv_loucal(a,c,emisToLocFile);break;case "Save simulation spectrum":c=
"Header:"+makeSimHeader();$("#chkSavePars").prop("checked")&&(c+="\nParameters:"+JSON.stringify(simParHeader()));$("#chkSaveSimu").prop("checked")&&(c=c+"\nResult:\n"+simulSaveDat());a=graphSettn.homFileN;seiv_loucal("Sim-"+a,c,emisToLocFile);break;case "Save material data to":a=$("#ediMater").val();a=a.slice(a.lastIndexOf("/")+1);c=matrlArr[0][0]+"\t"+matrlArr[0][1]+"\t"+matrlArr[0][2];matrlArr[0][3]&&(c+='\t"'+matrlArr[0][3]+'"');for(var b,d=1;d<matrlArr.length;d++)b=matrlArr[d][0]+"\t"+matrlArr[d][1]+
"\t"+matrlArr[d][2],c=c+"\r"+b;seiv_loucal(a,c,matrlToLocFile);break;case "Save target data to":a=$("#ediTarge").val();a=a.slice(a.lastIndexOf("/")+1);var c=targArr[0][0]+"\t"+targArr[0][1];targArr[0][2]&&(b=targArr[0][2].replace(/(^[\"\']+)|([\"\']+$)/g,""),c=c+"\t"+('"'+b+'"'));for(d=1;d<targArr.length;d++)b=targArr[d][0]+"\t"+targArr[d][1],c=c+"\r"+b;seiv_loucal(a,c,targetToLocFile);break;case "Save stack data to":a=$("#ediStack").val(),a=a.slice(a.lastIndexOf("/")+1),c=JSON.stringify(stack),seiv_loucal(a,
c,stackToLocFile)}$("#mongoDialForm").dialog("close")});$("#btnUserDir").click(function(){$("#frm-DirSel").hide();"No login"!=userName?(dirUser=userName,$("#frm-Login").hide(),$("#frm-DirSel").hide(),treeUpdate(),$("#frm-FileTree").show(),$("#FilTreLege").text("Files on server in "+userName+"'s directory"),$("#btn-mngOpenSave").show()):(dirUser="Publ",$("#frm-Login").show(),$("#frm-FileTree").hide(),$("#btn-mngOpenSave").hide())});
