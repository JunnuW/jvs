$("#mongoDialForm").dialog({open:function(){"Save emission spectrum"==$(this).dialog("option","title")?$("#saaveSim").css("display","inline"):$("#saaveSim").css("display","none");$("#frm-DirSel").show();$("#frm-Login").hide();$("#frm-FileTree").hide();$("#btn-mngOpenSave").hide();"No login"!=userName?$("#btnLogMeOff").show():$("#btnLogMeOff").hide();-1<$("#mongoDialForm").dialog("option","title").indexOf("Open")?($("#btn-mngOpenSave").text("Open File"),$("#btnPublDir").show()):($("#btn-mngOpenSave").text("Save data"),
$("#btnPublDir").hide())},autoOpen:!1,hide:{effect:"explode",duration:500},height:"auto",width:"auto",modal:!1,buttons:[{id:"btnMngoCancel",text:"Cancel","class":"medium-btn",click:function(){$("#mongoTree").jstree("deselect_all");$("#mongoTree").jstree("close_all");$("#FilTreLege").text("Files available on server:");var a=$(this).dialog("option","title"),b=$("#settnDial").dialog("option","title");"Inhomogeneous spectrum"==b&&"Open emission spectrum"==a?(inhomSpectr.experPlot=[],inhomSpectr.experArr=
[],graphSettn.inhomFileN="",$("#inhDescLbl").css("display","none"),$("#inhDesc").css("display","none"),$("#inhDesc").html(""),makeExpArrs(),inhombr()):"Homogeneous spectrum"==b&&"Open emission spectrum"==a&&(homSpectr.experPlot=[],homSpectr.experArr=[],simSettn.fileN="",$("#homDescLbl").css("display","none"),$("#homDesc").css("display","none"),$("#homDesc").html(""),makeExpArrs(),hombr());$(this).dialog("close")}},{id:"btnLogMeOff",text:"Log me off","class":"medium-btn",click:function(){$.get("/logout");
userName="No login";window.sessionStorage.setItem("RTFuser","");window.sessionStorage.setItem("RTFtoken","");$("#frm-FileTree").hide();$("#btnLogMeOff").hide()}},{id:"btn-mngOpenSave",text:"Open File","class":"medium-btn-inline",click:function(){var a=$("#btn-mngOpenSave").text();-1<a.indexOf("Open")?(a=$("#mongoFileName").val(),mongoGetOne(a,dirUser)):operDispatcher(a)}}]});
$("#chkSaveExper, #chkSavePars, #chkSaveSimu").click(function(){$("#btnUserDir").attr("disabled",!1);$("#btnLocDir").attr("disabled",!1);"chkSaveSimu"==$(this).attr("id")&&1==$(this).prop("checked")&&($("#chkSaveExper").prop("checked",!1),$("#numSel").prop("disabled",!1));"chkSaveExper"==$(this).attr("id")&&1==$(this).prop("checked")&&($("#chkSavePars").prop("checked",!1),$("#chkSaveSimu").prop("checked",!1),$("#numSel").prop("disabled",!0));"chkSavePars"==$(this).attr("id")&&1==$(this).prop("checked")&&
($("#numSel").prop("disabled",!0),$("#chkSaveExper").prop("checked",!1));1==$("#chkSaveSimu").prop("checked")?($("#btnUserDir").attr("disabled",!0),$("#numSel").prop("disabled",!1)):($("#btnUserDir").attr("disabled",!1),$("#numSel").prop("disabled",!0));0==$("#chkSaveSimu").prop("checked")&&0==$("#chkSavePars").prop("checked")&&0==$("#chkSaveExper").prop("checked")&&($("#btnUserDir").attr("disabled",!0),$("#btnLocDir").attr("disabled",!0),$("#numSel").prop("disabled",!0))});
...
function mongoSave(a,b){var d=pickCollection(),c;"No login"!=userName&&(c=window.sessionStorage.getItem("RTFtoken"));var e=DFmngo.dialog("option","title"),f={},g=$("#mongoFileName").val(),g=g.replace(/(^\/)|(\/$)/g,""),g=g.trim();if(1>g.length)f=toJsonArr(d,b,[],$("#mongFileDesc").val());else switch(g=$("#mongFileDesc").val(),e){case e.match(/material/gi)?e:void 0:f=toJsonArr(d,b,matrlArr,g);break;case e.match(/target/gi)?e:void 0:f=toJsonArr(d,b,targArr,g);break;case e.match(/emission/gi)?e:void 0:if(1==
$("#chkSaveExper").prop("checked")){var h=$("#lblsaveSimSpe").text();if("Inhomogeneous simulation"==h)f=toJsonArr(d,b,inhomSpectr.experArr,g);else if("Homogeneous simulation"==h)f=toJsonArr(d,b,homSpectr.experArr,g);else throw"Data saving error";}else f.Filename=b,f.Descr=g,f.Header=makeSimHeader(),f.Params=JSON.stringify(simParHeader());break;case e.match(/stack/gi)?e:void 0:g=g.replace(/(^[\"\']+)|([\"\']+$)/g,"");f.Filename=b;f.Descr=g;f.Stack=stack;break;default:throw"No datacollection for "+
e;}$.post(a,{userNme:dirUser,Collection:d,rtftoken:c,data:JSON.stringify(f)}).done(function(a){if(a&&200==a.statCode)if(-1<a.resString.indexOf("saving OK")||-1<a.resString.indexOf("Updated")){$("#btn-mngOpenSave").text("Save file");switch(e){case e.match(/material/gi)?e:void 0:$("#descMater").val($("#mongFileDesc").val());EnDisButt("Enabled","#btnUseMat");break;case e.match(/target/gi)?e:void 0:$("#descTarge").val($("#mongFileDesc").val());EnDisButt("Enabled","#btnUseTarg");break;case e.match(/stack/gi)?
e:void 0:$("#descStack").val($("#mongFileDesc").val());break;case e.match(/emission/gi)?e:void 0:break;default:throw"No datacollection for "+e;}$.notifyBar({cssClass:"success",html:"Your data was saved to file: "+f.Filename})}else $.notifyBar({cssClass:"warning",html:"File was not saved, database error: "+a.error})}).fail(function(a){handleFail(a.responseText,"Nothing was saved")});DFmngo.dialog("close")}
function saveToMngoDb(a){var b=pickCollection();$.post("/auth/checkOneUserF",{userNme:dirUser,Collection:b,fileName:a}).done(function(b){if(b&&200==b.statCode)switch(b.resString){case "yes":$("#btn-mngOpenSave").text("Confirm Overwrite");break;case "no":mongoSave("/auth/dbInsert",a);break;default:$.notifyBar({cssClass:"error",html:"File was not saved! bad response from server!"})}else $.notifyBar({cssClass:"error",html:b?b.error:"Could not check if already file exists on server!"}),DFmngo.dialog("close")}).fail(function(){$.notifyBar({cssClass:"error",
...
DFmngo.dialog("close")}
function pickCollection(){var a=DFmngo.dialog("option","title"),b;switch(a){case a.match(/material/gi)?a:void 0:b="materials";break;case a.match(/target/gi)?a:void 0:b="targets";break;case a.match(/stack/gi)?a:void 0:b="stacks";break;case a.match(/Save emission/gi)?a:void 0:$("#chkSaveExper").prop("checked")?b="emissions":$("#chkSavePars").prop("checked")&&(b="simulparams");break;case a.match(/Open emission/gi)?a:void 0:$("#radioMeas").prop("checked")?b="emissions":$("#radioSimu").prop("checked")&&(b=
"simulparams");break;default:throw"No datacollection for "+a;}return b}
function treeUpdate(){$("#directoName").prop("disabled",!1);$("#mongoFileName").prop("disabled",!1);$("#mongFileDesc").prop("disabled",!1);var a=pickCollection(),b;"No login"!=userName&&(b=window.sessionStorage.getItem("RTFtoken"));$.post("/auth/checkAllUserF",{userNme:dirUser,rtftoken:b,Collection:a}).done(function(a,b,e){a?"invalid"==a.token?(userName="No login",window.sessionStorage.setItem("RTFuser",userName),window.sessionStorage.setItem("RTFtoken",null),$.notifyBar({cssClass:"error",html:a.response}),
...
$("#mongoDialForm").dialog("close");$("#stackLocFiles").focus().click();break;case "Open emission spectrum":$("#mongoDialForm").dialog("close");$("#emisLocFiles").focus().click();break;case "Save emission spectrum":var b;a="Inhomogeneous spectrum"==$("#settnDial").dialog("option","title")?graphSettn.inhomFileN:graphSettn.homFileN;$("#chkSaveExper").prop("checked")?b=experEmisDat():(b="Header:"+makeSimHeader(),$("#chkSavePars").prop("checked")&&(b+="\nParameters:"+JSON.stringify(simParHeader())),$("#chkSaveSimu").prop("checked")&&
(b=b+"\nResult:\n"+simulSaveDat()));seiv_loucal("Sim-"+a,b,emisToLocFile);break;case "Save material data to":a=$("#ediMater").val();a=a.slice(a.lastIndexOf("/")+1);b=matrlArr[0][0]+"\t"+matrlArr[0][1]+"\t"+matrlArr[0][2];matrlArr[0][3]&&(b+='\t"'+matrlArr[0][3]+'"');for(var d,c=1;c<matrlArr.length;c++)d=matrlArr[c][0]+"\t"+matrlArr[c][1]+"\t"+matrlArr[c][2],b=b+"\r"+d;seiv_loucal(a,b,matrlToLocFile);break;case "Save target data to":a=$("#ediTarge").val();a=a.slice(a.lastIndexOf("/")+1);b=targArr[0][0]+
"\t"+targArr[0][1];targArr[0][2]&&(d=targArr[0][2].replace(/(^[\"\']+)|([\"\']+$)/g,""),b=b+"\t"+('"'+d+'"'));for(c=1;c<targArr.length;c++)d=targArr[c][0]+"\t"+targArr[c][1],b=b+"\r"+d;seiv_loucal(a,b,targetToLocFile);break;case "Save stack data to":a=$("#ediStack").val(),a=a.slice(a.lastIndexOf("/")+1),b=JSON.stringify(stack),seiv_loucal(a,b,stackToLocFile)}$("#mongoDialForm").dialog("close")});
