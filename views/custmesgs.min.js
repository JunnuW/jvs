...
function msgTreeUpdate(){msgHeader.prop("disabled",!1);msgText.prop("disabled",!1);$.post("/auth/getMessages",{moderated:"all"}).done(function(a,b,c){a?200==a.statCode?(bTreeData=JSON.parse(a.resString),msgTree.jstree(!0).settings.core.data=bTreeData,msgTree.jstree(!0).refresh()):$.notifyBar({cssClass:"error",html:a.error}):$.notifyBar({cssClass:"error",html:"could not obtain user messages from server"})}).fail(function(){$.notifyBar({cssClass:"warning",html:"(Connection or database error)"})})}
